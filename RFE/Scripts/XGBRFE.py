...
for train,test in kf.split(data,outcome):
    ...
    sys.stdout.write(str(i)+"th fold: CV RMSLE = "+str(rmsle_inner)+"/n")
    sys.stdout.flush()
    i += 1
    rmsle += rmsle_inner
...
pickle.dump(selection_result,open("XGBRFE-{}-rawmode.pkl".format(num),"wb"))
pickle.dump(best_model,open("XGBRFE-{}}-rawmodel.pkl".format(num),"wb"))
